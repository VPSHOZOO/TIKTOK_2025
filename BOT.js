...
const tls = require('tls-client');
const fs = require('fs');
const path = require('path');
const { exec } = require('child_process');
const prettytable = require('prettytable');
const { unquote } = require('querystring');
...
class Zefoy {
    ...
    async find_video() {
        ...
        while (true) {
            ...
            try {
                this.video_info = Base64.decode(unquote(request.data.toString().split('').reverse().join('')));
    ...
    async use_service() {
        ...
        let res;
        try {
            res = Base64.decode(unquote(request.data.toString().split('').reverse().join('')));
        } catch (e) {
            await new Promise(resolve => setTimeout(resolve, 3000));
            return "";
        }
        ...
        if (this.service === 'Comments Hearts') {
            ...
            try {
                res = Base64.decode(unquote(commentRequest.data.toString().split('').reverse().join('')));
            } catch (e) {
                await new Promise(resolve => setTimeout(resolve, 3000));
                return "";
            }
...
bot.onText(/🎥 Play TikTok Video/, (msg) => {
    ...
    bot.once('message', async (msg) => {
        if (msg.text && msg.text.includes('tiktok.com')) {
            ...
            bot.sendMessage(chatId, 'That doesn\'t look like a TikTok URL. Please try again.');
        }
    });
});
    ...
    const options = {
        reply_markup: JSON.stringify({
            inline_keyboard: [
                ...
                [{ text: 'Set Video URL', callback_data: 'set_video_url' }],
                [{ text: 'Set Comment ID', callback_data: 'set_comment_id' }]
            ]
        })
    };
...
bot.on('callback_query', async (callbackQuery) => {
    ...
    if (data === 'change_service') {
        bot.sendMessage(chatId, 'Current service: ' + zefoy.service + '\nPlease send the new service name:');
        
        bot.once('message', async (msg) => {
            zefoy.service = msg.text;
            zefoy.change_config();
            bot.sendMessage(chatId, `Service changed to: ${zefoy.service}`);
        });
    } else if (data === 'set_video_url') {
        bot.sendMessage(chatId, 'Current video URL: ' + zefoy.url + '\nPlease send the new video URL:');
        
        bot.once('message', async (msg) => {
            zefoy.url = msg.text;
            await zefoy.get_video_id();
            zefoy.change_config();
            bot.sendMessage(chatId, `Video URL changed to: ${zefoy.url}`);
        });
    } else if (data === 'set_comment_id') {
        bot.sendMessage(chatId, 'Current comment ID: ' + (zefoy.comment_id || 'Not set') + '\nPlease send the new comment ID:');
        
        bot.once('message', async (msg) => {
            zefoy.comment_id = msg.text;
            zefoy.change_config();
            bot.sendMessage(chatId, `Comment ID changed to: ${zefoy.comment_id}`);
        });
    }
});
